			...
			<div className="w-full h-full overflow-auto" style={{ backgroundColor: "#1A1B1C" }}> {/*Right Grey Side*/}
				...
				{currentSong &&
					<div className='text-white flex flex-row sticky bottom-0 bg-black border border-red-600 rounded-lg w-full h-20'>
						<div className='bg-cover bg-center w-20 h-20 p-2 ml-1'>
							<img src={currentSong.thumbnail} alt="" className='rounded-lg h-full w-full' />
						</div>
						<div className='flex flex-col text-white justify-center ml-3'>
						...
						<div className='flex flex-col w-2/5 ml-36'>
							<div className='flex flex-row justify-center space-x-6 mt-3'>
								<Icon icon="mi:previous" color="red" width="28" height="28" className='cursor-pointer' />
								<Icon icon={isPaused ? "zondicons:play-outline" : "zondicons:pause-outline"} color="red" width="28" height="28" className='cursor-pointer' onClick={togglePlayPause} />
								<Icon icon="mi:next" color="red" width="28" height="28" className='cursor-pointer' />
							</div>
							<div>

							</div>
						</div>
						<div className='bg-red-600 w-1/5 ml-32'></div>
					</div>
				}
			</div>
		</div>
